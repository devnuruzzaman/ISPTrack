    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.configuration.boxes.update', $box) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="sub_zone_id" id="sub_zone_id" class="form-control @error('sub_zone_id') is-invalid @enderror" required>
                                ...
                                @foreach($subZones as $subZone)
                                    <option value="{{ $subZone->id }}"
                                            {{ old('sub_zone_id', $box->sub_zone_id) == $subZone->id ? 'selected' : '' }}>
                                        {{ $subZone->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="code" id="code" class="form-control @error('code') is-invalid @enderror"
                                   value="{{ old('code', $box->code) }}" required>
                            @error('code')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="location" id="location" class="form-control @error('location') is-invalid @enderror"
                                   value="{{ old('location', $box->location) }}" required>
                            @error('location')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="capacity" id="capacity" class="form-control @error('capacity') is-invalid @enderror"
                                   value="{{ old('capacity', $box->capacity) }}" required>
                            @error('capacity')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
